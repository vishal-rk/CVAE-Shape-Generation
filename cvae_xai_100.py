...
XAI_DIR = f'{BASE_DIR}/cvae_512_xai_100'
...
def create_latent_traversals(model, epoch, class_idx, device, latent_size, n_steps=8, n_dims=100, traversal_range=(-5, 5)):
    """
...
def create_reconstruction_error_heatmaps(model, epoch, data_loader, device, n_samples_per_class=1):
    ...
    Args:
        ...
        device: Device to run the model on
        n_samples_per_class: Number of samples to create heatmaps for
    """
    ...
    # Create a custom colormap - black for high error, white for low error
